...
import { buildDocumentMetadata } from '../seo';
type PageParams = { params: Promise<{ slug: string[] }> };
export async function generateMetadata({ params }: PageParams) {
  const { slug } = await params;
  const doc = await getDocument('/' + slug.join('/'));
  if (!doc) return { title: 'Page not found — IndusAGI', robots: { index: false, follow: false } };
  const description = doc.html.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim().slice(0, 160);
  const canonical = doc.canonicalPath || '/' + slug.join('/');
  return buildDocumentMetadata(doc.title, description, canonical);
}
