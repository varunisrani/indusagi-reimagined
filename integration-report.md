...
The generated route catalog contains 179 routes: 166 canonical document URLs, six compatibility area roots, and seven preserved editorial/use-case pages. Each area root permanently redirects to one preferred canonical document. Native Next.js resolves those redirects before React streaming and statically bounds the catch-all to known documents, so aliases return real HTTP 308 responses and unknown routes return 404 instead of soft-200 pages. The XML sitemap contains 174 unique URLs including `/`; the six redirect-only area roots are excluded.
...
- `next.config.ts`, `app/[...slug]/page.tsx`: framework-level permanent redirects for native Next/Vercel, Vinext-compatible render redirects, and static parameters that bound native Next to the 173 non-alias generated documents.
- `app/site-origin.ts`, `app/layout.tsx`: absolute canonical metadata derived from a validated automatic `*.vercel.app` production hostname, then a validated HTTPS site override, then the existing `https://www.indusagi.com` fallback; request host headers are never trusted.
...
- `app/robots.txt/route.ts`, `app/sitemap.xml/route.ts`, `public/llms.txt`: deployment-aware crawler URLs, deduplicated sitemap, and source-qualified LLM discovery support with only a registry-verified TypeScript installation command.
...
- `tests/docs-integration.test.mjs`, `tests/native-next-runtime.test.mjs`: source pin, per-area counts, representative routes, exhaustive local path/fragment integrity, area-root canonical identity, release entrypoint modes, source-qualified LLM discovery, and a real native Next production build/server regression covering redirects, not-found status, traversal probes, canonical origin trust, robots, sitemap, and LLM discovery.
...
- `npm test` — PASS; 9/9 tests, including a native Next.js production build and loopback server.
...
- Native `next build` plus `next start` HTTP regression — PASS; 173 non-alias catch-all pages were statically generated, all six aliases returned 308 with the expected location, unknown routes returned 404, two encoded traversal probes returned 400/404, root and representative documents returned 200, trusted deployment canonical/robots/sitemap origins were absolute and host-header independent, and `/llms.txt` returned 200.
- `npm run build` — PASS; all five Vinext production-build stages completed with Next.js 16.3.3. Vinext runtime compatibility checks returned the same six 308 redirects, a 404 unknown route, and a 200 preferred document.
- `git diff --check` — PASS.
- Six explicit `diff -qr <canonical-area> <destination-area>` comparisons — PASS with no differences.
- Production runtime checks — PASS with HTTP 200 for `/`, preferred and nested pages in all six documentation families, `/robots.txt`, `/sitemap.xml`, and `/llms.txt`; unknown and encoded traversal paths returned 400/404 under the asserted native semantics.
...
- Native Next.js 16.3.3 logs its internal `NoFallbackError` diagnostic when `dynamicParams = false` rejects an unknown catch-all path; the wire response is the required 404 and contains the custom not-found page rather than a streamed soft-200 response.
