...
Canonical documentation source is stored as Markdown-compatible `.txt` files under `content/`. `content/source.json` pins the upstream repository and commit. `pnpm docs:build` safely renders that corpus into the route-split files under `app/data/`; homepage FAQs remain in `app/data/faq.json`. Statements from the source are retained, including the differing npm package names in the installer and one FAQ answer.
...
Static page content renders on the server. Shared navigation persists across client transitions. Documentation search is isolated from article rendering, and generated content is loaded through independent per-route imports. `dev` and `build` regenerate routes from the pinned corpus; run `pnpm docs:build` directly after editing `content/`. Npm counts load near the viewport and are cached for fifteen minutes.

The September refresh takes layout inspiration from Mastra: a spacious near-black canvas, pill controls, a floating 3D hero, and tabbed capability panels. The identity and content remain IndusAGI. The five code examples are excerpts from the preserved documentation, with working copy controls and deep links. The canonical `marked` renderer is the only dependency added by the docs integration.
